...
GOLD_KEEP = """
  ...
  /* Hero wordmark as on the live atlasglinn.com ("look at details matter on the Atlas Glinn website and apply the same ... non-static"):
     "Details" carries the moving gold shimmer, "Matter." is Atlas blue. */
  h1.mega .gold { background:linear-gradient(90deg, #BF953F 0%, #FCF6BA 25%, #B38728 50%, #FBF5B7 75%, #AA771C 100%); background-size:1000px 100%; animation:shimmer 6s linear infinite; -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; text-shadow:0 0 80px rgba(201,168,76,.3); }
  h1.mega .white { color:#1A6BDE; }
  /* Owner, 2026-09-04: the wordmark "is too large and takes away from the main screen" — about a third smaller than the shell's size. */
  h1.mega { font-size:clamp(2.2rem, 4.6vw, 4.4rem); letter-spacing:.03em; margin-bottom:1.6rem; }
