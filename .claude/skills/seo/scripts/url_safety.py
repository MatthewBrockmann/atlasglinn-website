...
def _trusted_proxy_hosts() -> frozenset:
    """
    Hostnames of egress proxies the operator configured via environment
    variables (HTTPS_PROXY / HTTP_PROXY / ALL_PROXY, upper- and lower-case).

    Sandboxed environments (CI runners, Claude Code remote containers)
    force ALL outbound traffic through a proxy that often listens on a
    loopback address. The proxy is transport, not target: the requested
    URL has already passed ``validate_url_strict``, so resolving the
    operator-configured proxy host — even to a private IP — is not an
    SSRF vector this module can or should block. Only hosts named in the
    proxy env vars are exempted; a redirect to ``127.0.0.1`` is still
    refused.
    """
    hosts = set()
    for var in (
...
def _pin_dns(hostname: str, pinned_ip: str, port: int) -> Iterator[None]:
    ...
    def patched(host, requested_port, *args, **kwargs):
        ...
        # Branch 2: the operator-configured egress proxy resolves without
        # the safety predicate — it is transport, not target (see
        # _trusted_proxy_hosts). requests connects to the proxy host, not
        # the (already strictly validated) target, so refusing a loopback
        # proxy here would break every fetch in proxied sandboxes.
        if host and str(host).lower() in _trusted_proxy_hosts():
            return original_getaddrinfo(host, requested_port, *args, **kwargs)

        # Branch 3: every OTHER hostname (redirect target, embedded
