...
if( function_exists( "WC" ) ){
    ...
    if( $enable_woocommerce == "yes" && ! empty( $woocommerce_product_id ) ){
        ...
        if( ! empty( $woocommerce_products ) ){
            $use_wc = true;
            $woocommerce_product = get_product( $woocommerce_products[0]->ID );
    ...
    <div id="yit-popup-border" class="group <?php if($use_wc): ?> woocommerce<?php endif ?>">
        ...
        <div id="yit-popup-right" class="<?php echo ( $use_wc ) ? 'product ' : '' ?> <?php echo ( ! $show_left ) ? 'yit-popup-full' : '' ?>">
            ...
            <?php if( $use_wc ): ?>
                ...
                <div class="add_to_cart"><a class="btn btn-flat" href="<?php echo $yit_addtocart_url?>"><?php echo $popup_addtocart_text?></a></div>
            <?php elseif( $enable_form == 'yes' ): ?>
                <?php echo do_shortcode( '[newsletter_form post_name="' . $form_name . '"]' ) ?>
            <?php endif; ?>
        </div>
        <?php endif; ?>
