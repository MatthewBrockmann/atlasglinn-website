...
class YIT_Backup_Reset {
    ...
    public function delete_cache_data() {
        ...
        $echo = true;
        if ( $action && ( $action == 'import-sampledata' || $action == 'install-sampledata' ) ) {
            $echo = false;
        }
        ...
        if ( $echo ) {
            if ( $die ) {
                ...
                //theme update
                YIT_Registry::get_instance()->message->addMessage( __( 'Theme updated successfully!', 'yit' ) );
                YIT_Registry::get_instance()->message->printGlobalMessages();
            }
        }
    }
    ...
    public function restore_backup() {
        ...
        if ( empty( $wp_filesystem ) && ! WP_Filesystem() ) {
			YIT_Registry::get_instance()->message->addMessage( __( 'Cannot access to the filesystem', 'yit' ), 'error', 'panel' );
        ...
        if ( ! isset( $_POST['backup_name'] ) ) {
            ...
            $options     = unserialize( base64_decode( $wp_filesystem->get_contents( $backup_file ) ) );
    ...
    protected function _import_data() {
        ...
        if( YIT_Request()->post('sampledata') == 'true' ){
            ...
            $content_file = $wp_filesystem->get_contents( $file );
        }
