...
export function statementsFromWorker() {
  ...
  return {
    ...
    BURN: grab('burnPendingCode', /prepare\('(UPDATE pending_signups SET verify_code_hash = \?, verify_expires_at = \?, verify_attempts = \?, code_sent_at = \? WHERE address_digest = \? AND verify_code_hash IS NOT NULL)'\)/),
...
export function run(db) {
  ...
  // schema.sql is the fresh-install shape; migrations/010 is the same table for a database that already exists.
  ...
  add('schema.sql and migrations/010 create the SAME pending_signups columns — a fresh install and an existing database agree',
  ...
  add('a second sign-up REPLACES the pending row rather than adding one — the credentials that become an account are the ones whose code was mailed last',
      after.length === 1 && after[0].password_hash === 'hash-of-the-owners-password' && after[0].name === 'Vic Owner' && after[0].verify_attempts === 0,
  ...
  add('the burn clears the code AND the one-a-minute stamp, so the owner is not also made to wait',
      db.changes(S.BURN, [null, null, 0, null, D1]) === 1 && !db.get(S.READ, [D1]).verify_code_hash && !db.get(S.READ, [D1]).code_sent_at, 'burned');
  add('… and burning twice reports nothing the second time — one racing guess is told it did the burning, not five',
      db.changes(S.BURN, [null, null, 0, null, D1]) === 0, 'idempotent');
